...
from uam.db.models import Agent, Reputation
...
async def session_factory():
    """Create an in-memory async engine with SQLModel tables and yield a session factory."""
...
async def session(session_factory):
    """Yield a single session for direct DB operations in tests."""
    async with session_factory() as sess:
        yield sess


@pytest.fixture()
def mgr(session_factory):
    """Create a ReputationManager backed by the session factory."""
    return ReputationManager(session_factory)
...
class TestTiers:
    ...
    @pytest.mark.asyncio
    async def test_get_tier_full(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_get_tier_full_at_100(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_get_tier_reduced(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_get_tier_throttled(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_get_tier_blocked(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_get_send_limit_by_tier(self, session, mgr):
...
class TestScoreUpdates:
    ...
    @pytest.mark.asyncio
    async def test_update_score_positive(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_update_score_negative(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_update_score_clamped_at_100(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_update_score_clamped_at_0(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_set_score_admin_override(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_set_score_clamps_over_100(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_set_score_clamps_below_0(self, session, mgr):
...
class TestCacheLoading:
    ...
    async def test_load_cache_from_db(self, session, session_factory):
        ...
        rep = Reputation(address=addr, score=75)
        session.add(rep)
        await session.commit()

        mgr = ReputationManager(session_factory)
        await mgr.load_cache()
        assert mgr.get_score(addr) == 75

    @pytest.mark.asyncio
    async def test_load_cache_clears_old_entries(self, session_factory):
...
class TestReputationInfo:
    ...
    @pytest.mark.asyncio
    async def test_get_reputation_info(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_get_reputation_info_not_found(self, mgr):
        """Returns None for unknown address."""
        info = await mgr.get_reputation_info("ghost::unknown.com")
        assert info is None

    @pytest.mark.asyncio
    async def test_record_message_sent_increments(self, session, mgr):
    ...
    @pytest.mark.asyncio
    async def test_record_message_rejected_increments(self, session, mgr):
